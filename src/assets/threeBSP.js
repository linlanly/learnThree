...
class Polygon {
  ...
  tessellate = function (poly) {
    ...
    return returning((polys = []), function () {
      if (f.length >= 3) {
        polys.push(new Polygon(f));
      }
      if (b.length >= 3) {
        return polys.push(new Polygon(b));
      }
    });
  };

  subdivide = function (polygon, coplanar_front, coplanar_back, front, back) {
...
class Node {
  ...
  clone = function () {
    var node,
      _this = this;
    return returning((node = new Node()), function () {
  ...
  build = function (polygons) {
    ...
    return returning(this, function () {
      ...
      if (_this.divider == null && polygons.length > 0) {
...
class Vertex {
  constructor(x, y, z, normal, uv) {
    __extends(Vertex, THREE.Vector3);
...
class ThreeBSP {
  ...
  toTree = function (treeIsh) {
    var face, geometry, i, polygons, _fn, _i, _len, _ref, _uv, _normal,
      _this = this;
    ...
    _normal = geometry.attributes.normal
    ...
    _fn = function (face, i) {
      ...
      for (vIndex = _j = 0, _len1 = _ref2.length; _j < _len1; vIndex = ++_j) {
        ...
        if ((idx = face[vName]) != null) {
          ...
          vertex = new Vertex(vertex.x, vertex.y, vertex.z, face.vertexNormals[0], new THREE.Vector2(faceVertexUvs[vIndex].x, faceVertexUvs[vIndex].y));
    ...
    polygon = new Polygon();
    for (i = _i = 0, _len = _ref.count; _i < _len; i = ++_i) {
      var faceVertexUvs, polygon, vertex, _ref1;
      faceVertexUvs = (_ref1 = geometry.faceVertexUvs) != null ? _ref1[0][i] : void 0;
      if (faceVertexUvs == null) {
        faceVertexUvs = [new THREE.Vector2(), new THREE.Vector2(), new THREE.Vector2(), new THREE.Vector2()];
      }
      vertex = new Vertex(_ref.getX(i), _ref.getX(i), _ref.getX(i), new THREE.Vector3(_normal.getX(i), _normal.getY(i), _normal.getZ(i)), new THREE.Vector2(_uv.getX(i), _uv.getY(i)));
      vertex.applyMatrix4(_this.matrix);
      polygon.vertices.push(vertex);
    }
  ...
  toMesh = function (material) {
    ...
    matrix.set(...this.matrix.elements)
    matrix.invert()
