...
function createSomething() {
  ...
  plane = createMesh(new THREE.TorusGeometry(10, 10, 8, 6, 2 * Math.PI))
  scene.add(plane)
  ...
  if (doc) {
    ...
    let keys = ['radius', 'tube', 'arc', 'radialSegments', 'tubularSegments']
    ...
    controls = Object.assign(controls, {
      redraw() {
        scene.remove(plane)
        plane = createMesh(
          new THREE.TorusGeometry(Math.floor(controls.radius), Math.floor(controls.tube),
            Math.floor(controls.radialSegments), 
            Math.floor(controls.tubularSegments),
            controls.arc))
        scene.add(plane)
      }
    })
    ...
    let gui = initGUI()
        gui.add(controls, 'radius', -10, 40).onChange(controls.redraw);
        gui.add(controls, 'tube', -10, 40).onChange(controls.redraw);
        gui.add(controls, 'arc', -4 * Math.PI, 4 * Math.PI).onChange(controls.redraw);
        gui.add(controls, 'radialSegments', -10, 10).onChange(controls.redraw);
        gui.add(controls, 'tubularSegments', -10, 10).onChange(controls.redraw);
