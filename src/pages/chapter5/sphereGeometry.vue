...
function createSomething() {
  ...
  plane = createMesh(new THREE.SphereGeometry(4, 10, 10))
  scene.add(plane)
  ...
  if (doc) {
    ...
    controls = Object.assign(controls, {
      ...
      redraw() {
        scene.remove(plane)
        plane = createMesh(
          new THREE.SphereGeometry(controls.radius, 
            Math.floor(controls.widthSegments), 
    ...
    let gui = initGUI()
        ...
        gui.add(controls, 'phiLength', 0, 4 * Math.PI).onChange(controls.redraw);
