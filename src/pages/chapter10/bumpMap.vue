...
let orbitControl, scene, mixer, clock
function createMesh(geometry, imageFile, bumpFile) {
  ...
  if (bumpFile) {
    const bump = new THREE.TextureLoader().load(`src/assets/general/${bumpFile}`)
  ...
  if (doc) {
    ...
    const controls = {
      ...
      changeTexture: function(e) {
        ...
        const bump = new THREE.TextureLoader().load(`src/assets/general/${e}-bump.jpg`)
    ...
    gui.add(controls, 'textureType', ['stone', 'weave']).onChange(controls.changeTexture)
