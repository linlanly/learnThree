...
function createSomething() {
  ...
  if (doc) {
    ...
    let keys = ['radius', 'tube', 'arc', 'radialSegments', 'tubularSegments', 'p', 'q', 'heightScale']
    ...
    controls = Object.assign(controls, {
      redraw() {
        scene.remove(plane)
        plane = createMesh(
          new THREE.TorusKnotGeometry(Math.floor(controls.radius), Math.floor(controls.tube),
            Math.floor(controls.radialSegments), 
            Math.floor(controls.tubularSegments),
            Math.floor(controls.p), 
            Math.floor(controls.q)
            ))
        scene.add(plane)
      }
    })
    ...
    let gui = initGUI()
        ...
        gui.add(controls, 'q', -10, 10).onChange(controls.redraw);
