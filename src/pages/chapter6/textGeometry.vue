...
import transfromSVGPathExposed from "@/assets/d3-threeD.js";
import { TextGeometry } from "three/addons/geometries/TextGeometry.js"
import { FontLoader } from "three/addons/loaders/FontLoader.js"
let controls = {}, scene, text1, text2, helvetiker = null, gentilis = null

function createMesh(geom) {
  let meshMaterial = new THREE.MeshPhongMaterial({ specular: 0xffffff, color: 0xeeffff, shininess: 40 })
...
function drawShape() {
  ...
  let shape = transfromSVGPathExposed(svgString, THREE)
  return shape
}
...
function createSomething() {
  ...
  camera.lookAt(new THREE.Vector3(10, 40, 30));

  let dirLight = new THREE.DirectionalLight()
  dirLight.position.set(25, 23, 15)
  scene.add(dirLight)

  let dirLight1 = new THREE.DirectionalLight()
  dirLight1.position.set(-25, 23, 15)
  scene.add(dirLight1)
  ...
  if (doc) {
    ...
    function fontsCompletelyLoadedBefore(callback) {
      function fontsCompletelyLoaded() {
        if (!helvetiker || !gentilis) {
          requestAnimationFrame(fontsCompletelyLoaded)
        } else {
          callback()
        }
      }
      fontsCompletelyLoaded()
    }
    fontsCompletelyLoadedBefore(() => {
      controls = {
        size: 90,
        depth: 9,
        bevelThickness: 2,
        ...
        redraw() {
          ...
          if (controls.font === 'helvetiker') {
            options.font = helvetiker
          } else {
            options.font = gentilis
          }
          ...
          text2 = createMesh(new TextGeometry('Three.js', options))
          scene.add(text2)
        }
      }
      let gui = initGUI()
      gui.add(controls, 'size', -100, 100).step(1).onChange(controls.redraw)
      gui.add(controls, 'depth', -10, 10).step(1).onChange(controls.redraw)
      gui.add(controls, 'font', ['gentilis', 'helvetiker']).onChange(controls.redraw)
      gui.add(controls, 'bevelThickness', -10, 10).step(1).onChange(controls.redraw)
      gui.add(controls, 'bevelSize', -10, 10).step(1).onChange(controls.redraw)
      gui.add(controls, 'bevelSegments', -10, 10).step(1).onChange(controls.redraw)
      gui.add(controls, 'bevelEnabled').step(1).onChange(controls.redraw)
      gui.add(controls, 'curveSegments', -5, 5).step(1).onChange(controls.redraw)
...
<template>
  ...
  <div id="batman" style="display:none">
    <svg version="1.0" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
      ...
      <g>
        <path id="batman-path" style="fill:rgb(0,0,0);"
