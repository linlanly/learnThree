...
let controls = {}, scene, orbit, tubeMesh

function createMesh(geom) {
  let meshMaterial = new THREE.MeshPhongMaterial({ specular: 0xaaaaff, color: 0x3399ff, shininess: 40 })
...
function createSomething() {
  ...
  if (doc) {
    ...
    let gui = initGUI()
    let klein = function (u, v, position) {
      u *= Math.PI;
      v *= 2 * Math.PI;
    ...
    let radialWave = function (u, v, position) {
      var r = 50;
    ...
    let mesh = createMesh(new ParametricGeometry(radialWave, 20, 60, false))
    scene.add(mesh)
